...
from webtiles import game_data_handler, util, ws_handler, status, bot_api
...
class HTTPSRedirectHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        if https_port is None:
            ...
            https_port = ""
        self.redirect(
            f"https://{self.request.host_name}{https_port}{self.request.uri}",
            permanent=True,
        )
...
def stop_everything(shutdown_event):
    ...
    def do_stop(tries=0):
        if len(ws_handler.sockets) == 0:
            ...
            logging.error(
                "Remaining sockets: %s", ws_handler.describe_sockets(names=True)
            )
            for task in all_tasks_compat():
                task.cancel()
        else:
            IOLoop.current().add_timeout(
                time.time() + 0.5, functools.partial(do_stop, tries=tries + 1)
            )
...
def bind_server(nonsecure_sockets, secure_sockets):
    ...
    handlers = [
        ...
        (r"/bot/state", bot_api.BotStateHandler),
        (r"/bot/input", bot_api.BotInputHandler),
        (r"/bot/log", bot_api.BotLogHandler),  # ✅ 이 줄 추가
    ...
    application = tornado.web.Application(
        handlers, gzip=config.get("use_gzip"), **settings
    )
    ...
    if secure_sockets:
        ...
        server = tornado.httpserver.HTTPServer(
            application, ssl_options=config.get("ssl_options"), **kwargs
        )
...
def parse_args_main():
    ...
    parser.add_argument(
        "--ssl-port",
        type=int,
        help="An SSL port to bind. Requires configured `ssl_options`.",
    )
    parser.add_argument("--logfile", help='A logfile to write to; use "-" for stdout.')
    parser.add_argument(
        "--daemon", action="store_true", default=None, help="Daemonize after start."
    )
    parser.add_argument(
        "-n",
        "--no-daemon",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Debug mode for server admins. Will use a separate directory for sockets. "
...
def export_args_to_config(args):
    if config.get("live_debug"):
        config.server_config._load_override_file(
            os.path.join(config.get("server_path", ""), "debug-config.yml")
        )
...
def reset_token_commands(args):
    ...
    if args.clear_reset:
        ...
        if not ok:
            err_exit(
                "Error generating password reset token for %s: %s" % (username, msg)
            )
        else:
            if not user_info.email:
                logging.warning(
                    "No email set for account '%s', use caution!" % username
                )
...
def mode_to_flag(mode):
    if mode == "admin":
        ...
        if not config.get("wizard_accounts"):
            logging.warning(
                "Wizard accounts not enabled on this server, do you want `admin`?"
            )
...
def show_flags(username):
    ...
    if not r.flags:
        ...
        print(
            "Flags for '%s' (id %d): %s"
            % (r.username, r.id, userdb.flag_description(r.flags))
        )
    return True
...
def flag_commands(args):
    ...
    if args.list:
        ...
        if r:
            err_exit(
                "Error when setting flag %s for '%s': %s" % (args.set, arg.flag, r)
            )
        else:
            print("Flag set.")
            show_flags(args.set)
        ...
        if r:
            err_exit(
                "Error when clearing flag %s for '%s': %s" % (args.clear, arg.flag, r)
            )
        else:
            print("Flag cleared.")
...
def ban_commands(args):
    if args.check_config_bans or args.run_config_bans:
        ...
        if err:
            err_exit(
                "Error when trying to set hold for user '%s': %s" % (args.hold, err)
            )
        else:
        ...
        if err1:
            ...
            if err2:
                err_exit(
                    "Error when trying to clear hold for user '%s': %s"
                    % (args.clear, err2)
                )
            else:
...
def parse_args_util():
    ...
    parser_ban.add_argument(
        "--add",
        type=str,
        help="Set an account ban for a user. Replaces any account holds.",
    )
    parser_ban.add_argument(
        "--hold",
        type=str,
        help="Set an account hold for a user. Replaces any ban flags.",
    )
    ...
    parser_ban.add_argument(
        "--list-holds", action="store_true", help="List current account holds."
    )
    parser_ban.add_argument(
        "--clear-holds", action="store_true", help="Clear all current account holds"
    )
    ...
    parser_flag.add_argument(
        "flag",
        type=str,
        nargs="?",
        default="",
        help="a flag to query with (ban, hold, admin, wizard, bot)",
    )
    parser_flag.add_argument(
        "--list", action="store_true", help="List any users with the indicated flag"
    )
    parser_flag.add_argument("--set", type=str, help="Set a flag on a specified user.")
    parser_flag.add_argument(
        "--clear", type=str, help="Clear a flag on a specified user."
    )
    parser_flag.add_argument(
        "--show",
        type=str,
        help="Show flags on a specified user (does not require [flag]).",
    )
...
def do_chroot():
    if config.get("chroot"):
        ...
        try:
            ...
            print(
                "Error: can't import `tornado.platform` in chroot. Did you copy"
